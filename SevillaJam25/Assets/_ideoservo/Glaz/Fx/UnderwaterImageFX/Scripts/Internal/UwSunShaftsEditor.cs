...
{
    [CustomEditor (typeof(UwSunShafts))]
    class SunShaftsEditor : Editor
    {
        ...
        public override void OnInspectorGUI () {
            ...
            if ((target as UwSunShafts).GetComponent<Camera>())
                GUILayout.Label("Current camera mode: "+ (target as UwSunShafts).GetComponent<Camera>().depthTextureMode, EditorStyles.miniBoldLabel);
            ...
            if ((target as UwSunShafts).sunTransform && (target as UwSunShafts).GetComponent<Camera>()) {
                if (GUILayout.Button("Center on " + (target as UwSunShafts).GetComponent<Camera>().name)) {
                    if (EditorUtility.DisplayDialog ("Move sun shafts source?", "The SunShafts caster named "+ (target as UwSunShafts).sunTransform.name +"\n will be centered along "+(target as UwSunShafts).GetComponent<Camera>().name+". Are you sure? ", "Please do", "Don't")) {
                        Ray ray = (target as UwSunShafts).GetComponent<Camera>().ViewportPointToRay(new Vector3(0.5f,0.5f,0));
                        (target as UwSunShafts).sunTransform.position = ray.origin + ray.direction * 500.0f;
                        (target as UwSunShafts).sunTransform.LookAt ((target as UwSunShafts).transform);
                    }
                }
            }
